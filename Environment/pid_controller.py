            ...
            for key, value in performance.items():
                if key == "episode_data":
                    serializable_perf[key] = {
                        ...
                        "pid_outputs": [
                            [[float(p) for p in pid_out] for pid_out in episode]
                            for episode in value["pid_outputs"]
                        ],
                    }
...
def visualize_pid_performance(
    ...
    """
    Create visualizations of PID controller performance with PID output analysis.
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Train SINDy model and evaluate PID controller"
    )
